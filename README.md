# Chainlink Random Creature Creation

(based on original [DnD Character repo](https://github.com/PatrickAlphaC/dungeons-and-dragons-nft))
...
We will easily create our own NFT on the Rinkeby Chain. We can edit the name of the creature in the [`generate-creature.js`](./scripts/generate-creature.js) script. 

This will create a creature with a level (1-3) and color values from 0 - 255:
 -   uint256 level;
 -   uint256 red;
 -   uint256 green;
 -   uint256 blue;
...
### Clone the tepo and migrate
```
git clone https://github.com/Ex-Populus/expoppler.git
cd expoppler
npm install
truffle migrate --reset --network rinkeby
```

This will deploy your ExPoppler NFT!

### Generate a creature
You can now try it out:
```bash
truffle exec scripts/fund-contract-creature.js --network rinkeby
truffle exec scripts/generate-creature.js --network rinkeby
truffle exec scripts/get-creature.js --network rinkeby
```

This will create a new creature with random a level and colors! 
Depending how often you deploy, you can pick which creature by changing the [`exp.getCreatureOverView(1)`](contracts/ExPoppler.sol) command in `get-creature.js` to swap the `0` out with whatever `tokenId` of the creature you like. 
...
truffle run verify ExPoppler --network rinkeby
```
...
Once we have our image, we can add it to our metadata `.json` file, and add our stats in there. You can see some samples in the `metadata` folder. We want to use the values of our creatures that we got off-chain, so be sure to verify what the random numbers you got on etherscan! Once we have the .json metadata file, we want to add that to IPFS as well, and pin it too!
...
Now, we can get the address of our NFT and head on over to the opensea testnet marketplace to see if we did it correctly. If done correctly, it'll look [something like this](https://testnets.opensea.io/assets/expoppler-v9).
